...
- **Deliverable:** `drone_model.py`, `_bridge.py`, real implementations of `connect`, `takeoff`, `land`, `hover`, `move`, `rotate`, `move_to`, and the command-queue/round-trip in `registry.py`. Kinematic motion at speeds mapped from `VelocityLevel` (define the m/s mapping in config). **Caution:** the pyhulax `VelocityLevel` enum value is a position-controller P-gain where *lower = faster* (SLOW=300, ZOOM=100, TURBO=50) — it is **not** a speed. Map level→speed by NAME via `config.velocity_levels` (TURBO fastest); never use the enum int as a speed. Blocking semantics per §5.4. Battery model (simple linear drain) feeding `get_battery`. Enforce `NotReady`/`LowBattery`.
